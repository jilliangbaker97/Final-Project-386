...
def get_etf_holdings(ticker: str) -> pd.DataFrame:
    """Scrape the top holdings and portfolio weights for an ETF.

    Fetches the Yahoo Finance holdings page for the given ETF and parses
    the holdings table.  Returns at most the top 25 rows that Yahoo
    typically exposes without authentication.
    ...
    ------
    ValueError
        If the page cannot be fetched, the holdings table is missing, or
        no rows are parsed.

    Notes
    -----
    Web scraping is inherently fragile.  If Yahoo changes their markup this
    function will raise a ``ValueError`` with a descriptive message.  For
    production use, consider the paid Yahoo Finance or a dedicated ETF-data
    API instead.
    ...
    """
    ticker = _validate_ticker(ticker)
    url = _ETF_SOURCES["default"].format(ticker=ticker)
    ...
    try:
        ...
        raise ValueError(
            f"HTTP request failed for ETF holdings ('{ticker}'): {e}"
        ) from e

    soup = BeautifulSoup(resp.text, "lxml")

    # Yahoo renders holdings in a <table> whose first <th> contains "Symbol"
    table = _find_holdings_table(soup)
    if table is None:
        raise ValueError(
            f"Holdings table not found for '{ticker}'. "
            "Yahoo Finance may have changed their page structure, "
            "or this ticker may not be an ETF."
        )
    ...
    for tr in table.find("tbody").find_all("tr"):
        ...
        if len(cells) < 3:
            continue
        symbol, name, weight_str = cells[0], cells[1], cells[2]
    ...
    log.info(
        "get_etf_holdings('%s'): %d holdings scraped.", ticker, len(result)
    )
    return result
